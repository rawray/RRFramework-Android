...
public class RefreshLayout extends FrameLayout {
    ...
    public RefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (attr == R.styleable.RefreshLayout_refresh_enabled_up) {
                    setRefreshEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.RefreshLayout_refresh_enabled_down) {
                    setLoadMoreEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.RefreshLayout_refresh_style_up) {
                    mHeadStyle = a.getInt(attr, CLASSIC);

                } else if (attr == R.styleable.RefreshLayout_refresh_style_down) {
                    mFootStyle = a.getInt(attr, CLASSIC);

                } else if (attr == R.styleable.RefreshLayout_refresh_friction) {

                    setFriction(a.getFloat(attr, DEFAULT_FRICTION));

                } else if (attr == R.styleable.RefreshLayout_refresh_duration) {

                    mDuration = a.getInt(attr, DEFAULT_DURATION);

                } else if (attr == R.styleable.RefreshLayout_refresh_smooth_duration) {

                    mSmoothDuration = a.getInt(attr, DEFAULT_SMOOTH_DURATION);

                } else if (attr == R.styleable.RefreshLayout_refresh_smooth_length) {

                    mSmoothLength = a.getInt(attr, DEFAULT_SMOOTH_LENGTH);

                } else if (attr == R.styleable.RefreshLayout_refresh_background_header) {

                    mRefreshBackgroundResource = a.getResourceId(attr, android.R.color.transparent);

                } else if (attr == R.styleable.RefreshLayout_refresh_background_footer) {

                    mLoadMoreBackgroundResource = a.getResourceId(attr, android.R.color.transparent);

                } else if (attr == R.styleable.RefreshLayout_refresh_is_coordinator) {
    ...
    private IRefresh getHeaderInterface() {
        return (IRefresh) mHeaderView;
    }

    private IRefresh getFooterInterface() {
        return (IRefresh) mFooterView;
    }
